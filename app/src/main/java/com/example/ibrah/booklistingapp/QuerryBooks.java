...
public class QuerryBooks {
    ...
    private static List<Book> extractFeatureFromJson(String BooksJSON) {
        ...
        try {
            ...
            JSONArray baseJsonResponseJSONArray = baseJsonResponse.getJSONArray("items");
            ...
            for (int i = 0; i < baseJsonResponseJSONArray.length(); i++) {
                ...
                JSONObject volumInfo = currentObjectInJasonArray.getJSONObject("volumeInfo");
                String Title = volumInfo.getString("title");
                StringBuilder authors = new StringBuilder();
                if (volumInfo.has("authors")) {
                    JSONArray autorsArrays = volumInfo.getJSONArray("authors");
                    for (int j = 0; j < autorsArrays.length(); j++) {
                        authors.append(autorsArrays.getString(j)).append(" ,");
                    }
                    //remove the las coma
                ...
                if (volumInfo.has("imageLinks")) {
                    JSONObject Thumbmail = volumInfo.getJSONObject("imageLinks");
                    if (Thumbmail.has("smallThumbnail")) {
                        thumbmail = Thumbmail.getString("smallThumbnail");
                    }
                }
                Log.i(LOG_TAG, " LINK OF THE IMAGE IS---------------- : " + i + ":  " + thumbmail);
                ///////////////
                String url = null;
                if (volumInfo.has("infoLink")) {
                    // Extract the value for the key called "infoLink"
                    url = volumInfo.getString("infoLink");
                }
                //////////
                String info = null;
                if (volumInfo.has("description")) {
                    info = volumInfo.getString("description");
                }
                String rating = null;
                if (volumInfo.has("averageRating")) {
                    rating = volumInfo.getString("averageRating");
                }

                // Add the new {@link Earthquake} to the list of Books.
                Log.i(LOG_TAG, Title + "" + authors.toString() + "" + rating + "" + info + "" + url);
                Books.add(new Book(Title, authors.toString(), rating, info, thumbmail));
            }
